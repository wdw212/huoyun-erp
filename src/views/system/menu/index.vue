<template>
	<div class="app-container">
		...
		<el-table v-if="refreshTable" v-loading="loading" :data="menuList" row-key="id"
			...
			<el-table-column prop="status" label="状态" align="center">
				<template #default="scope">
					...
					<el-tag
						:type="scope.row.status == 1 ? 'success' : 'danger'">{{ scope.row.status == 1 ? '启用' : '停用' }}</el-tag>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" align="center" width="300" class-name="small-padding fixed-width">
				<template #default="scope">
					...
					<el-button plain type="primary" icon="Edit" @click="handleAdd(scope.row)">新增</el-button>
		...
		<!-- 添加或修改菜单对话框 -->
		<el-dialog :title="title" v-model="open" width="900px" :modal="true">
					...
					<el-col :span="12">
						<el-form-item>
							...
							<el-radio-group v-model="form.status">
								<el-radio v-for="item in sys_normal_disable" :key="item.value"
...
<script setup name="Menu">
	...
	function getList() {
		...
		listMenu(queryParams.value).then(response => {
			// menuList.value = proxy.handleTree(response.data, "parent_id");
