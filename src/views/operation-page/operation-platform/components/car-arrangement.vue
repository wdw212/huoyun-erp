<template>
	...
	<div class="p-r" v-loading="loading">
		<!-- 顶部搜索 -->
		<search-top ref="searchTop" :queryParams="queryParamsCarArrangement" @search="searchConfirm"></search-top>
		...
		<!-- 单据详情 -->
		<el-dialog v-model="dialogFormVisible" title="排车详情" width="80%" :close-on-click-modal="false">
			<el-card>
...
<script setup name="CarArrangement">
	import {
		ref,
		onMounted,
		h,
	...
	import {
		queryParamsCarArrangement,
		formListCarArrangement,
		AccountsColumn,
		amountFormDoc,
		rulesInit
	} from '@/utils/platform';
	...
	onMounted(async () => {
		...
		formListNew.value = formListCarArrangement.value;
