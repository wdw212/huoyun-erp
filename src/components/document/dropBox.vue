...
<script setup>
	...
	const openDrop = (val, options) => {
		...
		saveData.value = val;
		showDrop.value = true;
		var boxInfo = Array.isArray(val.boxInfo)&&val.boxInfo.length>0?val.boxInfo[0]:{};
		
		var entered_port_wharf = val.entered_port_wharf_id&&options?.MT?options.MT.find(v=>{return v.id==val.entered_port_wharf_id})||{}:{};  //进港码头
		var shipping_company = val.shipping_company_id&&options?.CGS?options.CGS.find(v=>{return v.id==val.shipping_company_id})||{}:{};  //船公司
		...
		(val.boxInfo||[]).forEach((item,index)=>{
			var boxType = item.container_type_id&&options?.XZLX?options.XZLX.find(v=>{return v.id==item.container_type_id})||{}:{};  //箱型
			var lxdc = item.drop_off_wharf_id&&options?.LX?options.LX.find(v=>{return v.id==item.drop_off_wharf_id})||{}:{};  //落箱堆场
			...
			data2.value = data2.value + '\n' + `柜型：${boxType?.name||''}    箱号：${item.no||''}    封号：${item.seal_number||''}    ${['未进港','未进港——预录入已做','已进港'][parseInt(item.is_entered_port)]}`
		})
