...
export function timeToTimestamp(format) {
	var mydata = format.replace(/\-/g, '/');
	return new Date(mydata);
}

export function timeto(date, type, seper) {
	if (seper) {
	...
	if (seper == 'text') {
		if (type == "y") {
			...
			//转时分秒
			var currentdate = hours + '时' + minutes + '分' + seconds + '秒';
		} else if (type == "hm") {
			//转时分秒
			var currentdate = hours + '时' + minutes + '分';
		}
	} else {
		if (type == "y") {
			// 转年月
